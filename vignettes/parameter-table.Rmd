...
## Solution
The package `nonmemica` provides the function `as.partab` that creates a parameter table for a single NONMEM model.  `as.partab` knows where the models live, because you explicitly pass a ```project``` argument or (if you are lazy like me) you set an option near the top of your script.  
...
Making a NONMEM parameter table can be tedious.  The `nonmemica` package provides `as.partab`, providing a flexible way of assembling the main sources of information and presenting them nicely.  A good parameter table should have rich metadata: symbols, labels, and units at least.  `as.partab` supports several options for encoding metadata systematically.