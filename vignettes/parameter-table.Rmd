...
Making a NONMEM parameter table can be tedious.  The `nonmemica` package provides `as.partab`, a flexible way of assembling the main sources of information and presenting them nicely.  A good parameter table should have rich metadata: symbols, labels, and units at least.  `as.partab` supports several options for encoding metadata systematically.