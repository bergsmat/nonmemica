...
#Created 2017-10-05 at 13:55

rplots.level <- 1
xpose.runno <- ''
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'execute diagnostic plots 2001.ctl'
working.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2001/modelfit_dir3/'
raw.results.file <- 'raw_results_2001.csv'
model.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2001/'
model.filename<-'2001.ctl'
subset.variable<-NULL
mod.suffix <- '.ctl'
mod.prefix <- 'run'
tab.suffix <- ''
theta.labels <- c('CL/F','Vc/F','Ka')
