...
#Created 2017-10-05 at 14:01

rplots.level <- 1
xpose.runno <- ''
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'execute diagnostic plots 2003.ctl'
working.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2003/modelfit_dir1/'
raw.results.file <- 'raw_results_2003.csv'
model.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2003/'
model.filename<-'2003.ctl'
subset.variable<-NULL
mod.suffix <- '.ctl'
mod.prefix <- 'run'
tab.suffix <- ''
theta.labels <- c('CL/F','Vc/F','Ka','Q/F','Vp/F')
