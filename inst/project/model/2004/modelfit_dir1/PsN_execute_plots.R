...
#Created 2017-10-05 at 14:03

rplots.level <- 1
xpose.runno <- ''
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'execute diagnostic plots 2004.ctl'
working.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2004/modelfit_dir1/'
raw.results.file <- 'raw_results_2004.csv'
model.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2004/'
model.filename<-'2004.ctl'
subset.variable<-NULL
mod.suffix <- '.ctl'
mod.prefix <- 'run'
tab.suffix <- ''
theta.labels <- c('CL/F','Vc/F','Ka','Q/F','Vp/F','WT_CL')
theta.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE,TRUE)
omega.labels <- c('IIV_CL','IIV_Vc','IIV_Ka')
omega.fixed <- c(FALSE,FALSE,FALSE)
sigma.labels <- c('ERR_PROP','ERR_ADD')
