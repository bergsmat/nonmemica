...
#Created 2017-10-05 at 14:05

rplots.level <- 1
xpose.runno <- ''
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'execute diagnostic plots 2007.ctl'
working.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2007/modelfit_dir1/'
raw.results.file <- 'raw_results_2007.csv'
model.directory<-'/home/tbergsma/project/nonmemica/inst/project/model/2007/'
model.filename<-'2007.ctl'
subset.variable<-NULL
mod.suffix <- '.ctl'
mod.prefix <- 'run'
tab.suffix <- ''
theta.labels <- c('CL/F','Vc/F','Ka','Q/F','Vp/F','WT_CL','MALE_CL')
theta.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)
omega.labels <- c('IIV_CL','CL_V','IIV_Vc','CL_Ka','Vc_Ka','IIV_Ka')
